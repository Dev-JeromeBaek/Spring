...
(function(a) {
	...
	c || (f = a.fn.val, a.fn.val = function() {
		...
		if (this.length) {
			...
			this.hasClass("placeholder") && this.removeClass("placeholder");return f.apply(this, arguments)
		}
	});
	a.fn.placeholderEnhanced = function() {
		...
		if (this.length) return c || a("form").filter(function() {
					...
					if (!a.data(this, "placeholderEnhanced")) {
						...
						}).insertBefore(b));b.bind("blur.placeholder", function() {
							b.val() || (c ? b.addClass("placeholder") : j ? (e.css({
								position : "",
								left : ""
							}), k(b)) : b.val(h).addClass("placeholder"))
						}).bind("focus.placeholder", g).trigger("blur.placeholder");a.data(d, "placeholderEnhanced", !0)
					}
				})
	};a(function() {
