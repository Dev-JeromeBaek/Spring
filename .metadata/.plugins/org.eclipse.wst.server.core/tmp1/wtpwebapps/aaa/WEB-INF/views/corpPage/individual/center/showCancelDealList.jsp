...
<body data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-46" data-genuitec-path="/aaa/src/main/webapp/WEB-INF/views/corpPage/individual/center/showCancelDealList.jsp">
	<h1 data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-46" data-genuitec-path="/aaa/src/main/webapp/WEB-INF/views/corpPage/individual/center/showCancelDealList.jsp">show Approve And Cancel</h1>
	
	<c:choose>
	<c:when test="${ !cancelDealList.isEmpty() }">
		<c:forEach var="i" items="${cancelDealList}">
			<figure class="snip1268">
					<div class="image">
						<img src="/aaa/resources/files/${i.di_sysName.split(',')[0]}" alt="sq-sample4"/>
							<div class="icons">
							</div>
						<a onclick="showDealDetail('${ i.di_num }')" class="add-to-cart"
	...
	<c:otherwise>
		현재 ${ corpLogin.getC_name() } 의 취소된 딜이 존재하지 않습니다.<br>
		<a href="/aaa/idvCorp/dealInsert">딜 신청하러 가기~!</a>
	</c:otherwise>
	</c:choose>
	<script type="text/javascript">
		function showDealDetail(num) {
			var url = "/aaa/idvCorp/dealCancelMsg?di_num=" + num;
