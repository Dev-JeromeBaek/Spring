...
<body data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-45" data-genuitec-path="/aaa/src/main/webapp/WEB-INF/views/corpPage/individual/center/showApproveDealList.jsp">
	<h1 data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-45" data-genuitec-path="/aaa/src/main/webapp/WEB-INF/views/corpPage/individual/center/showApproveDealList.jsp">show Approve And Cancel</h1>
	
	<c:choose>
	<c:when test="${ !approveDealList.isEmpty() }">
		<c:forEach var="i" items="${approveDealList}">
			<figure class="snip1268">
					<div class="image">
						<img src="/aaa/resources/files/${i.di_sysName.split(',')[0]}" alt="sq-sample4"/>
							<div class="icons">
							</div>
						<a href="/aaa/idvCorp/AllLocal?num=${i.di_num}" class="add-to-cart">보러가기</a>
					</div>
					<figcaption>
						<h2>${i.di_name}</h2>
						<p>${i.di_exp}</p>
	...
	<c:otherwise>
		현재 ${ corpLogin.getC_name() } 의 판매중인 딜이 없습니다.<br>
