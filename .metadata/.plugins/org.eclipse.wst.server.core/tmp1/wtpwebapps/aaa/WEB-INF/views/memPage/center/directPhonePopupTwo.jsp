...
<body data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-107" data-genuitec-path="/aaa/src/main/webapp/WEB-INF/views/memPage/center/directPhonePopupTwo.jsp">
<c:choose>
...
<c:otherwise>
	<table style="text-align: center; margin: auto;" >
		...
		<tr class="directPhonepwjl">
			...
			<td class="directPhonepwjl">${info.di_name}</td>
		</tr>
		<tr class="directPhonepwjl">
	...
	<input type="button" value="취소" onclick="nono()" class="pphoneTwoPorup">

	<input type="button" class="pphoneTwoPorup" value="인증요청" onclick="gogo('${totalprice}','${mnum}','${infonum}','${coco}','${discountpoint}','${point}')" >
</center>
	<table style="margin: auto; display: none;" class="plusPhone">
		...
		<tr class="directPhonepwjl">
			...
			<input class="pphoneTwoPorup" type="button" value="인증하기" onclick="whatPhone('${totalprice}','${mnum}','${infonum}','${coco}','${discountpoint}','${point}')"></td>
		</tr>
	</table>
	</c:otherwise>
	</c:choose>
...
<script type="text/javascript" >
	...
	function gogo(totalprice,mnum,infonum,coco,discountpoint,point) {
		...
		if(document.getElementById("phone_know_Three").value == ""){
			...
			if(parseInt(document.getElementById("phone_know_One").value) == phoneNumber[0]
				...
				"onclick='whatPhone(\""+totalprice.trim()+"\""+","+"\""+mnum.trim()+"\""+","+"\""+infonum.trim()+"\""+","+"\""+coco.trim()+"\""+","+"\""+discountpoint+"\""+","+"\""+point+"\")'></td></tr></table>";
				}
		}
		
	}
	
	function whatPhone(totalprice, mnum, infonum, coco,discountpoint,point) {
		...
		if (pwValue == "2751") {
			...
			window.opener.location.href =
				"/aaa/mem/directBuyAllSuccess?totalprice="+totalprice+"&mnum="+mnum+"&infonum="+infonum+"&coco="+coco
