...
<body style="text-align: center;" class="bodybidy" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-105" data-genuitec-path="/aaa/src/main/webapp/WEB-INF/views/memPage/center/directCardPopup.jsp">
<c:choose>
...
<c:otherwise>
	...
	<table style="text-align: center; margin: auto;" >
		...
		<td class="cardpopTRtd">상품명</td>
			<td class="cardpopTRtd">${info.di_name}</td>
		</tr>		
		<tr class="cardpopTRtd">
	...
	<div class="demo">
		<form class="payment-card">
			...
			<div class="payment-card__footer">
				<button class="payment-card__button">결제취소</button>
				<input type="button" value="결제하기" class="payment-card__button"
					onclick="payment_card('${totalprice}','${mnum}','${cardCbank}','${infonum}','${coco}','${discountpoint}','${point}')">
			</div>
		</form>
	</div>
...
<script type="text/javascript">
	...
	alert("totalprice : "+totalpriceOne);
	...
	function payment_card(totalprice,mnum,cardCbank,infonum,coco,discountpoint,point) {
		...
		"/aaa/mem/directBuyAllSuccess?totalprice="+totalprice+"&cardCbank="+cardCbank+"&mnum="+mnum+"&infonum="+infonum+"&coco="+coco+"&discountpoint="+discountpoint+"&point="+point;
	self.close();
		}
	}
