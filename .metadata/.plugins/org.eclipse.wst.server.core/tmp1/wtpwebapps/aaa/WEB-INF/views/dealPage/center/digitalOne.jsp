...
<body data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-58" data-genuitec-path="/aaa/src/main/webapp/WEB-INF/views/dealPage/center/digitalOne.jsp">

<section id="seTop" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-58" data-genuitec-path="/aaa/src/main/webapp/WEB-INF/views/dealPage/center/digitalOne.jsp">
      <!-- Page Content -->
        ...
        <div class="row">
			...
			<div class="row1">
	            ...
	            <div class="col-md-4" style="width: 500px; height: 300px;">
	                ...
	                <option id="c${each.dd_num }" value="${each.dd_num },${each.dd_kind },${each.dd_name },${each.dd_price},${each.dd_amount}">${each.dd_kind } - ${each.dd_name }</option><br>
	                </c:forEach>
	                </select>
	                </c:if>
	                <ul id="cartList"></ul><br>
					<ul id="totalprice"><li> <span>총 구매 가격 : </span>
										<span>0</span>
										<span>원</span></li>
					</ul><br>
		            <div id="detailBtns">
	...
	aa : for(var b=0;b<ccList.length;b++){
		...
		for(var qq=0;qq<getList.length;qq++){
			...
			if(parseInt(ddInfos[1])==parseInt(ppList[0])){
				nn+=${dealInfo.di_num}+","+ppList[0]+","+(parseInt(ccList.item(b).getElementsByTagName("input").item(1).value)+parseInt(ddInfos[2]))+","+ppList[3]+"-";
				continue aa;
			}
		}
		nn+=${dealInfo.di_num}+","+ppList[0]+","+ccList.item(b).getElementsByTagName("input").item(1).value+","+ppList[3]+"-";
	}
...
//		 	alert(qnaCon);
			location.href="/aaa/appliance_digital/insertQna?di_num=${dealInfo.di_num}&qnaCont="+qwer;
		}
