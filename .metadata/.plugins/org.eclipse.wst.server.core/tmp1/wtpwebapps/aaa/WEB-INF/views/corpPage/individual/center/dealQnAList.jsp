...
<body data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-38" data-genuitec-path="/aaa/src/main/webapp/WEB-INF/views/corpPage/individual/center/dealQnAList.jsp">
  ...
  <c:choose>
	<c:when test="${!diList.isEmpty()}"> 
		...
		<div class="image">
			...
			<a href="/aaa/idvCorp/dealQnA?num=${i.di_num}" class="add-to-cart">보러가기</a>
		</div>
		<figcaption>
			<h2>${i.di_name}</h2>
			<p>${i.di_exp}</p>
			<c:forEach var="cc" items="${DQCountList }">
			<c:if test="${cc.inte1==i.di_num }">
			<div class="price">${cc.inte2}</div>
			</c:if>
			</c:forEach>
		</figcaption>
	</figure>
