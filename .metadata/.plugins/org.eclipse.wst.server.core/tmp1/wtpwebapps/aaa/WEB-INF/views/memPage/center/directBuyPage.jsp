...
<body style="text-align: left;" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-103" data-genuitec-path="/aaa/src/main/webapp/WEB-INF/views/memPage/center/directBuyPage.jsp">
<c:choose>
...
<c:otherwise>
	...
	<h2>
		<font color="F57878" style="font-weight: bolder;" size="10"  >${info.di_exp} &nbsp; [ ${info.di_name} ]</font>상품을 주문합니다. 
	</h2>
	<h2><br>
	...
	<table border="1" class="AllBordersTrr">
		...
		<tr class="AllBordersTrrr"><td class="AllBordersTrrr">${info.di_name}</td>
...
- 8개 VAN사:(주)코밴, KIS정보통신, NICE정보통신, 브이피㈜, 한국신용카드결제(주), 퍼스트데이터코리아, ㈜케이에스넷, 스타VAN </td>
	...
	<input type="button" name="yes" value="구매하기" onclick="yesyes('${totalprice}','${mem.m_num}','${info.di_num}','${coco}')" class="button-three-gogo">
	<br></center>
	<br>
...
<script type="text/javascript">
	...
	function yesyes(totalprice,mnum,infonum,coco) {
		...
		var pluspage = "?totalprice="+totalprice+"&cbank="+cbank+"&cardCbank="+cardCbank+"&mnum="+mnum+"&infonum="+infonum+"&coco="+coco+"&discountpoint="+salePoint+"&point="+toPoint;
		obj = document.getElementsByName('ex1');
		if(obj[0].checked){
			window.open("/aaa/mem/directCardPopup"+pluspage,"","width=600, height=450,left=400 ,top=100");
 		}else if(obj[1].checked){
 			window.open("/aaa/mem/directPhonePopup"+pluspage,"","width=500, height=400,left=550 ,top=180");
 		}else if(obj[2].checked){
 			window.open("/aaa/mem/directAccountPopup"+pluspage,"","width=500, height=400,left=550 ,top=180");
 		}else if(obj[3].checked){
 			location.href ="/aaa/mem/directBuySecess"+pluspage;
 		}
