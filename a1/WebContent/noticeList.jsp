...
<spring:url value="/resources/files/yeob/httpRequest.js"
	var="httpRequest_js" />
	...
	<table>
		...
		<c:if test="${ !list.isEmpty() }">
			...
			<c:choose>
				...
				<c:otherwise>
					<td><a href="/aaa/notice/noticeMod?notice_num=${each.notice_num}">${each.notice_num }</a></td>
				</c:otherwise>
			</c:choose>
...
<!--  		<input type="button" value="검색" onclick="javascript:goSearch()"> -->
	...
	<script type="text/javascript">
		function goModi(notice_num){
			...
			frm.method = "get";
			frm.action = "/aaa/notice/noticeMod?notice_num="+notice_num;
			frm.submit();
		}
		function goInsertForm(){
			var frm = document.frm;
			frm.method = "get";
			frm.action = "/aaa/notice/noticeInsertForm";
			frm.submit();
		}
		...
		function goLog(){
			var frm = document.frm;
			frm.method = "get";
			frm.action = "/aaa/notice/adminLogin";
			frm.submit();
		}
...
// 		}
	...
	function go(notice_num){
		...
		sendRequest("/aaa/notice/noticeAnyView",params,callback,null);
