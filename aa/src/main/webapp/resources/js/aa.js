...
!function(a, b) {
		...
		cleanData : function(a, b) {
			for (var d, e, f, g, h = 0, i = n.expando, j = n.cache, k = l.attributes, m = n.event.special; null != (d = a[h]); h++)
				if ((b || M(d)) && (f = d[i], g = f && j[f])) {
					...
					j[f] && (
					delete j[f]
					, k || "undefined" == typeof d.removeAttribute ? d[i] = void 0 : d.removeAttribute(i), c.push(f))
			}
		}
	}), n.fn.extend({
		domManip : Ha,
	...
	n.extend({
		...
		ajax : function(b, c) {
			...
			i = n.event && l.global, i && 0 === n.active++ && n.event.trigger("ajaxStart"), l.type = l.type.toUpperCase(), l.hasContent = !Lb.test(l.type), f = l.url, l.hasContent || (l.data && (f = l.url += (Fb.test(f) ? "&" : "?") + l.data,
			delete l.data
				...
				send : function(d, e) {
					...
					g.send(b.hasContent && b.data || null), c = function(a, d) {
						...
						if (c && (d || 4 === g.readyState))
							if (
								delete fc[h]
								, c = void 0, g.onreadystatechange = n.noop, d) 4 !== g.readyState && g.abort();else {
...
;(function($) {
	$.fn.pgwMenu = function(options) {
		...
		var init = function() {
			pgwMenu.config = $.extend({}, defaults, options);setup();pgwMenu.checkMenu();pgwMenu.window.resize(function() {
				pgwMenu.plugin.css('overflow', 'hidden');clearTimeout(pgwMenu.resizeEvent);
				pgwMenu.resizeEvent = setTimeout(function() {
					pgwMenu.checkMenu();
				}, 100);
			});pgwMenu.plugin.find('.pm-dropDown').click(function(e) {
				pgwMenu.enableMobileDropDown();e.stopPropagation();
			});pgwMenu.plugin.find('.pm-viewMore').click(function(e) {
				pgwMenu.enableViewMoreDropDown();e.stopPropagation();
			});$(document).click(function() {
				pgwMenu.disableMobileDropDown();pgwMenu.disableViewMoreDropDown();
			});
		};
		var setup = function() {
			...
			pgwMenu.plugin.removeClass().addClass('pm-links');pgwMenu.plugin.wrap('<div class="' + wrapClass + '"></div>');
			pgwMenu.plugin = pgwMenu.plugin.parent();pgwMenu.plugin.prepend('<div class="pm-dropDown"><a href="javascript:void(0)">' + pgwMenu.config.dropDownLabel + '</a></div>');
		...
		pgwMenu.checkMenu = function() {
			...
			function getContentWidth() {
				...
				pgwMenu.plugin.find('.pm-links').removeClass('mobile').show();pgwMenu.plugin.find('.pm-links > li').each(function() {
			...
			function switchMenu(type) {
				if (type == 'viewmore') {
					...
					pgwMenu.plugin.find('.pm-links').removeClass('mobile').show();pgwMenu.plugin.find('.pm-viewMore > ul > li').remove();pgwMenu.plugin.find('.pm-links > li').show().each(function() {
						if (viewMoreMenuWidth + $(this).outerWidth(true) < pluginMaxWidth) {
							viewMoreMenuWidth += $(this).outerWidth(true);
						} else {
							pgwMenu.plugin.find('.pm-viewMore > ul').append($(this).clone().show());$(this).hide();
						}
					});pgwMenu.plugin.find('.pm-dropDown, .pm-viewMore > ul').hide();pgwMenu.plugin.find('.pm-viewMore').show();
				} else if (type == 'dropdown') {
					pgwMenu.plugin.find('.pm-links > li').show();pgwMenu.plugin.find('.pm-links').addClass('mobile').hide();pgwMenu.plugin.find('.pm-viewMore, .pm-viewMore > ul').hide();pgwMenu.plugin.find('.pm-viewMore > ul > li').remove();pgwMenu.plugin.find('.pm-dropDown').show();
				} else {
					pgwMenu.plugin.find('.pm-links > li').show();pgwMenu.plugin.find('.pm-links').removeClass('mobile').show();pgwMenu.plugin.find('.pm-dropDown, .pm-viewMore, .pm-viewMore > ul').hide();pgwMenu.plugin.find('.pm-viewMore > ul > li').remove();
				}
		...
		pgwMenu.enableViewMoreDropDown = function() {
			if (pgwMenu.plugin.find('.pm-viewMore > a').hasClass('active')) {
				pgwMenu.disableViewMoreDropDown();return false;
			}
			pgwMenu.plugin.find('.pm-viewMore > a').addClass('active');pgwMenu.plugin.find('.pm-viewMore > ul').show();
		};
		pgwMenu.disableViewMoreDropDown = function() {
			if (pgwMenu.plugin.find('.pm-viewMore > a').hasClass('active')) {
				pgwMenu.plugin.find('.pm-viewMore > a').removeClass('active');pgwMenu.plugin.find('.pm-viewMore > ul').hide();
			}
		};
		pgwMenu.enableMobileDropDown = function() {
			if (pgwMenu.plugin.find('.pm-dropDown > a').hasClass('active')) {
				pgwMenu.disableMobileDropDown();return false;
			}
			pgwMenu.plugin.find('.pm-dropDown > a').addClass('active');pgwMenu.plugin.find('.pm-links').show();
		};
		pgwMenu.disableMobileDropDown = function() {
			if (pgwMenu.plugin.find('.pm-dropDown > a').hasClass('active')) {
				pgwMenu.plugin.find('.pm-dropDown > a').removeClass('active');pgwMenu.plugin.find('.pm-links').hide();
			}
		};init();return this;
	}
...
window.Rainbow = function() {
	...
	function r(a, b, c, i) {
		if ("undefined" === typeof a || null === a) i();else {
			var e = a.exec(c);
			if (e) {
				++s;!b.name && "string" == typeof b.matches[0] && (b.name = b.matches[0],
				delete b.matches[0]
				);
				var k = e[0],
					g = e.index,
								...
								if (d) {
									var g = b.matches[c[a]],
										...
										j = function(b, d, g) {
											...
											d = g ? q(g, d) : d;
											k = k.substr(0, f) + k.substr(f).replace(b, d);l(++a, c, i)
										};
									f ? n(d, f, function(a) {
										j(d, a)
...
Rainbow.addClass = Rainbow.a;Rainbow.extend([ {
	matches : {
		1 : [ {
...
(function(a) {
	a.fn.pgwSlider = function(n) {
			...
			l.resizeEvent = setTimeout(function() {
				...
				if (typeof p != "undefined" && p && l.config.maxHeight == null) {
					if (typeof l.plugin.find(".ps-current").animate == "function") {
						...
						l.plugin.find(".ps-current").css("height", o);l.plugin.find(".ps-list > li").css("height", q)
					}
				} else {
					l.plugin.find(".ps-current").css("height", o);l.plugin.find(".ps-list > li").css("height", q)
				}
		...
		var e = function() {
			l.plugin.removeClass(l.config.mainClassName).addClass("ps-list");l.plugin.wrap('<div class="' + l.config.mainClassName + '"></div>');
			l.plugin = l.plugin.parent();l.plugin.prepend('<div class="ps-current"><ul></ul><span class="ps-caption"></span></div>');
			...
			if (l.config.displayControls && l.slideCount > 1) {
				l.plugin.find(".ps-current").prepend('<span class="ps-prev"><span class="ps-prevIcon"></span></span>');l.plugin.find(".ps-current").append('<span class="ps-next"><span class="ps-nextIcon"></span></span>');l.plugin.find(".ps-current .ps-prev").click(function() {
					l.previousSlide()
			...
			if (!l.config.displayList) {
				l.plugin.find(".ps-current").css("width", "100%");l.plugin.find(".ps-list").hide()
			}
			var o = 1;
			l.plugin.find(".ps-list > li").each(function() {
				var q = b(a(this));
				q.id = o;l.data.push(q);a(this).addClass("elt_" + q.id);
				if (q.title) {
			...
			i(1);l.plugin.find(".ps-current > ul > li.elt_1 img").on("load", function() {
				c();
				var p = l.plugin.find(".ps-current > ul > li.elt_1 img").height();
				f(p);l.window.resize(function() {
					c();
					...
					try {
						if (p.originalEvent.touches[0].clientX && l.touchFirstPosition != null) {
							if (p.originalEvent.touches[0].clientX > (l.touchFirstPosition + 50)) {
								l.touchFirstPosition = null;l.previousSlide()
							} else {
								if (p.originalEvent.touches[0].clientX < (l.touchFirstPosition - 50)) {
									l.touchFirstPosition = null;l.nextSlide()
								}
							}
						}
					} catch (p) {
		...
		var j = function(o) {
			...
			if (q != "") {
				...
				if (typeof l.plugin.find(".ps-caption").fadeIn == "function") {
					l.plugin.find(".ps-caption").html(q);l.plugin.find(".ps-caption").fadeIn(l.config.transitionDuration / 2)
				} else {
					l.plugin.find(".ps-caption").html(q);l.plugin.find(".ps-caption").show()
				}
			}
		...
		var d = function(r) {
			...
			l.plugin.find(".ps-list > li").css("opacity", "0.6");l.plugin.find(".ps-list > li.elt_" + r.id).css("opacity", "1");q.find("li").not(".elt_" + l.currentSlide).not(".elt_" + r.id).each(function() {
			...
			p.css("position", "absolute").show().animate({
				...
				p.css("position", "").css("z-index", 2).show();j(r)
			});return true
		};
		var k = function(s, v) {
			...
			if (l.currentSlide == 0) {
				r.find(".elt_1").css({
					...
					opacity : 1,
					"z-index" : 2
				}).show();l.plugin.find(".ps-list > li.elt_1").css("opacity", "1");j(s)
			} else {
				...
				q.css("position", "absolute").css("left", t).css("opacity", 1).show().animate({
					...
					l.transitionInProgress = false;l.plugin.find(".ps-list > li").css("opacity", "0.6");l.plugin.find(".ps-list > li.elt_" + s.id).css("opacity", "1");j(s)
				})
			}
			l.currentSlide = s.id;return true
		};
		...
		l.startSlide = function() {
			l.config.autoSlide = true;h();return true
		};
		l.stopSlide = function() {
			l.config.autoSlide = false;clearInterval(l.intervalEvent);return true
		};
		...
		l.destroy = function(o) {
			...
			if (typeof o != "undefined") {
				l.plugin.find(".ps-list > li").each(function() {
					a(this).attr("style", null).removeClass().css("cursor", "").unbind("click").unbind("mouseenter");a(this).find("a").css("cursor", "");a(this).find("img").attr("style", null)
				});l.plugin.find(".ps-list").addClass(l.config.mainClassName).removeClass("ps-list");l.plugin.find(".ps-current").unwrap().remove();l.hide()
			} else {
				l.parent().remove()
			}
			l.plugin = null;
		...
		l.reload = function(o) {
			...
			l = this;
			l.plugin = this;
			l.window = a(window);l.plugin.show();
			l.config = a.extend({}, g, o);e();
			if (l.config.autoSlide) {
				h()
			}
			return true
